...
### Changed — compressed before encrypted

- Files that aren't already compressed (text, code, CSV, JSON, office formats excluded, images/audio/video/archives/PDF skipped) are deflated in the tab before sealing when that saves at least 10 %: `CompressionStream("deflate-raw")`, so no library. The manifest event records `compression: "deflate-raw"` and `stored_size` (what the chunk framing covers); `size` stays the real size, and `openObject` inflates and checks it. Per version — an update that doesn't compress drops the flag. Share links and export carry the fields. **Cross-surface:** crate-agent 1.4.0 reads and writes the same format (fixtures both ways); an older daemon fails closed on a compressed object rather than mirroring deflated bytes — update it before enabling this build on a paired folder. Tests: `test/compression.test.mjs`, `test/crate-vault.test.mjs`.

### Added — passkey unlock on this device

- **Backup → Set up a passkey**: a WebAuthn passkey with the PRF extension (recent Chrome, Safari, Android, iOS, 1Password, Bitwarden) whose per-credential secret seals a device-local record — connection details + content key — in IndexedDB. The start screen then offers **Unlock with passkey · <folder>**: Touch ID / Face ID / Windows Hello / a security key opens the folder with no passphrase typed. Nothing is written to the bucket; other devices still use the passphrase or the credentials file; losing the device loses the shortcut only. A passkey session can change the passphrase and set up a recovery phrase (v1.1 folders) but not re-key or download the credentials file; a re-key re-seals the record (one prompt) or drops it. `Crate.open({ contentKey })` is the ESM entry it uses. Needs a pass on a real device — the browser pane here has no authenticator, so the flow was exercised with a stubbed `navigator.credentials`.

### Added — re-key a folder

- **Backup → Re-key folder** replaces the folder's content key: every file's data key is unwrapped and re-wrapped under a fresh key, the whole manifest is re-signed under it with a trailing `rekey` event (generation n+1), and `crate.json` is rewritten — objects untouched, nothing re-uploaded. Manifest first, then the key file, both with `If-Match`; if the key file write fails the old manifest is written back so the bucket never disagrees with itself. The recovery slot cannot survive (its entropy is not held), so the new phrase is shown immediately after; the search index is rebuilt. For a leaked key, or to fully retire the old passphrase of a folder migrated from v1.0.
- **Rollback anchor knows generations.** A re-signed chain looks like a fork to every other device's anchor; it is accepted only when its `rekey` generation is higher than the anchor's (a bucket-only attacker cannot mint that event, and replaying the pre-re-key bucket brings the old generation back). Same rule in crate-agent ≥ 1.4.0, which unwraps the new key from `passphrase_wrap` on its next start — older daemons need re-pairing. ESM: `crate.rekey({ passphrase })`. Tests in `test/crate-vault.test.mjs`.
