...
export function planExport(source, opts = {}) {
  ...
  const files = [];
  let totalBytes = 0;
  const only = opts.paths ? new Set(opts.paths) : null; // subset (bulk download)
...
async function readOne(source, fileMeta) {
  ...
  // openObject handles v1 blob vs v2 chunked and pins the leading IV to
  // the manifest-signed content_iv (2026-05 audit H1) so a rolled-back
  // object is rejected instead of written into the backup as stale
  // plaintext. planExport already carries content_iv + chunk_size.
  ...
  try {
    return await cryptoLib.openObject(dataKey, got.body, fileMeta);
  } catch (e) {
    throw new Error(`export: ${fileMeta.uuid}: ${e.message}`);
  } finally {
...
export async function runExport(source, plan, opts = {}) {
  ...
  const suggestedName = opts.suggestedName || `crate-backup-${today}.zip`;
