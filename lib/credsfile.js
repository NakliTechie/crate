...
//   {
//     "v":          1,
//     "provider":   "r2" | "carrier",             // future: "b2","hetzner","aws-s3"
//     "bucket":     { "name", "accountId", "region" [, "url"] },
//     "credentials":{ "accessKey", "secretKey" }
//   }
//
// provider "carrier": bucket.url is the crate-carrier Worker URL,
// bucket.region is "carrier", accountId is the literal "carrier", name is
// the Worker host (display only), and credentials.secretKey is the
// Worker's CARRIER_SECRET. Same envelope, same passphrase wrap.
...
export async function pack(creds, passphrase) {
  const carrier = creds?.provider === "carrier";
  ...
  if (carrier && !creds?.bucket?.url) throw new CredsFileError("pack: carrier creds need bucket.url");
  ...
  const inner = {
    ...
    bucket: {
      ...
      region: creds.bucket.region || (carrier ? "carrier" : "auto"),
    },
    credentials: {
  ...
  if (carrier) inner.bucket.url = creds.bucket.url;
...
export async function unpack(input, passphrase) {
  ...
  try {
    ...
    const carrier = inner.provider === "carrier";
    if (carrier && typeof inner.bucket.url !== "string") {
      throw new CredsFileError("unpack: carrier creds missing bucket.url");
    }
    const out = {
      ...
      bucket: {
        ...
        region: inner.bucket.region || (carrier ? "carrier" : "auto"),
      },
      credentials: {
    ...
    if (carrier) out.bucket.url = inner.bucket.url;
    return out;
  } finally {
    cryptoLib.zero(fileKey);
  }
