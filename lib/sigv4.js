...
/**
 * Presign a GET/HEAD with AWS Signature V4 query-string auth — a URL that
 * carries its own authorisation for `expiresSeconds` (max 7 days). Used
 * for share links: the recipient fetches ciphertext with a plain GET and
 * no credentials. Payload is UNSIGNED-PAYLOAD (there is none); the only
 * signed header is `host`.
 *
 * Verified against the AWS reference vector (examplebucket/test.txt,
 * 86400 s, 2013-05-24) in test/sigv4-presign.test.mjs.
 */
export async function presignUrl(opts) {
  ...
  if (!url) throw new Error("presignUrl: url is required");
  if (!region) throw new Error("presignUrl: region is required");
  if (!accessKey || !secretKey) throw new Error("presignUrl: accessKey + secretKey required");
  const expires = Math.floor(expiresSeconds);
  if (!(expires >= 1 && expires <= 604800)) throw new Error("presignUrl: expiresSeconds must be 1..604800");
  ...
  const stringToSign = [ALGORITHM, amzDate, scope, await sha256Hex(canonRequest)].join("\n");
  ...
  // Emit the query exactly as canonicalised (sig-v4 encoding), then the
  // signature last.
