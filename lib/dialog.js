...
export function formDialog({
  ...
  return new Promise((resolve) => {
    ...
    const cancelBtn = node("button", { type: "button", class: "btn btn-secondary" }, [cancelLabel]);
    const okBtn = node("button", { type: "submit", class: danger ? "btn btn-danger-primary" : "btn btn-primary" }, [confirmLabel]);
    card.appendChild(node("div", { class: "dialog-actions" }, [cancelBtn, okBtn]));
    ...
    check();
    const first = card.querySelector("[data-autofocus]") || focusables()[0];
    if (first) {
      first.focus();
