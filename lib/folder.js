...
export class FolderUI {
  ...
  // back to the bucket with If-Match against session.manifestETag (M6.x
...
async function buildIntentPayload(session) {
  ...
  return {
    ...
    identity_pubkey: "browser-stub", // M7.1: real Ed25519 pubkey from a per-browser Identity
    issued_at: now,
    expires_at: now + 900,
  };
}
