...
export class FolderUI {
  ...
  buildHeader() {
    ...
    // Top line: total file count + size summary so the user has
    // continuous visibility into how much they're storing.
    const stats = this.computeStats();
    wrap.appendChild(h("p", { class: "muted small" }, [
      `${stats.fileCount} ${stats.fileCount === 1 ? "file" : "files"} · ${formatBytes(stats.totalBytes)} total · end-to-end encrypted`,
    ]));
    ...
    const pairBtn = h("button", { type: "button", class: "btn btn-secondary" }, ["Pair an agent"]);
    pairBtn.addEventListener("click", () => this.handlePair());

    const exportBtn = h("button", { type: "button", class: "btn btn-secondary" }, ["Export folder"]);
    exportBtn.addEventListener("click", () => this.handleExport());

    wrap.appendChild(h("div", { class: "folder-actions row" }, [
      uploadBtn, mkdirBtn, refreshBtn, pairBtn, exportBtn, fileInput,
    ]));

    // Search box — filters the current tree view by basename substring.
    // Cleared on currentDir change. Re-rendered (not focused) on each
    // render to preserve typing; the input keeps its own focus state.
    const searchInput = h("input", {
      type: "search", class: "input", id: "folder-search",
    ...
    searchInput.addEventListener("input", (e) => {
      ...
      // Re-render only the tree, not the header, so the input keeps focus.
  ...
  buildTree() {
    ...
    if (entries.length === 0) {
      const label = this.searchQuery.trim()
        ? `(no matches for "${this.searchQuery.trim()}")`
        : "(empty)";
      wrap.appendChild(h("li", { class: "folder-empty muted" }, [label]));
      return wrap;
    }
  ...
  buildTreeRow(entry) {
    ...
    if (!entry.isDir && previewable(entry)) {
      const prevBtn = h("button", { type: "button", class: "btn-sm btn-secondary", title: "Preview" }, ["👁"]);
      prevBtn.addEventListener("click", () => this.handlePreview(entry));
    ...
    if (!entry.isDir) {
      const histBtn = h("button", { type: "button", class: "btn-sm btn-secondary", title: "View history" }, ["⏱"]);
      histBtn.addEventListener("click", () => this.handleHistory(entry));
