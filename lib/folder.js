...
export class FolderUI {
  ...
  buildHeader() {
    ...
    // Lock — zeros the master key + passphrase + session credentials,
    // clears the in-tab sessionStorage blob, and returns to the welcome
    // screen. The user has to re-unlock (credentials file or 5-input
    // form) to get back in.
    const lockBtn = h("button", { type: "button", class: "btn btn-danger-secondary btn-utility folder-lock-btn", title: "Lock the folder and return to the start screen" }, [
      "🔒 Lock",
    ]);
    ...
    wrap.appendChild(h("div", { class: "folder-toolbar" }, [
      ...
      h("div", { class: "toolbar-utility" }, [refreshBtn, credsBtn, pairBtn, exportBtn, lockBtn]),
    ]));
  ...
  async handleDownload(entry) {
    ...
    try {
      ...
      // Pin decryption to the manifest-signed content_iv to prevent a
      // bucket-only attacker from replaying older object bytes for the
      // same UUID. See lib/crate.js::read for the rationale.
      if (entry.entry?.content_iv) {
        const expected = cryptoLib.fromBase64(entry.entry.content_iv);
        if (!constantTimeIvEqual(iv, expected)) {
          throw new Error("object IV does not match manifest content_iv (rollback or tamper)");
        }
      }
  ...
  //
  // Anchor handling parallels Crate._flushManifest (lib/crate.js):
  //   - On successful PUT: advance the saved rollback anchor to the
  //     new manifest tail. Future loaders on this device require at
  //     least this count + matching prev_sig at that index.
  //   - On 412 re-fetch: validate the re-fetched manifest against the
  //     anchor BEFORE trusting it. A bucket-only attacker who induces
  //     the 412 (by racing a PUT) could swap to an older valid
  //     manifest during the re-GET; the anchor catches it. See the
  ...
  async flushManifest() {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (put.ok) {
        ...
        this.session.lastFlushedEventCount = this.session.manifest.events.length;
        // Advance the rollback anchor. Best-effort — IDB / sessionStorage
        // both failing is rare and non-fatal (next flush retries; worst
        // case a future load re-TOFUs at the current tail).
        try {
          await anchor.saveAnchor(this.session.bucketBase, this.session.manifest.tail());
      ...
      if (put.preconditionFailed && attempt < maxRetries) {
        ...
        // Anchor check on the re-fetched manifest. A bucket-only attacker
        // who races a PUT to induce our 412 could serve an older valid
        // manifest in the re-GET; AES-GCM + prev_sig both pass on the
        // prefix, so this is the layer that catches it.
  ...
  async handlePreview(entry) {
    ...
    try {
      ...
      if (size <= PREVIEW_SIZE_CAP) {
        ...
        const iv = get.body.subarray(0, 12);
        const ct = get.body.subarray(12);
        // See handleDownload for rationale: bind preview decryption to
        // manifest-signed content_iv to detect bucket-side rollback.
        if (entry.entry?.content_iv) {
          const expected = cryptoLib.fromBase64(entry.entry.content_iv);
          if (!constantTimeIvEqual(iv, expected)) {
            throw new Error("object IV does not match manifest content_iv (rollback or tamper)");
...
// constantTimeIvEqual compares two byte arrays (used to verify the
// object's leading IV against the manifest's signed content_iv field
// before decryption). 12-byte IVs make timing variance negligible in
// practice, but we use the standard XOR-fold pattern for hygiene.
