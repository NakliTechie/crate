...
import { mintShare, SHARE_TTL } from "./share.js";
...
import {
  ...
  createEvent, updateEvent, deleteEvent, moveEvent, mkdirEvent, purgeEvent, restoreEvent,
} from "./manifest.js";
...
// Deleted files keep their bytes in the bucket for this long; the trash
// view lists them and can restore them. Expired items are purged (object
// deleted, `purge` event appended) by whichever browser opens the folder.
...
function previewable(entry) {
  ...
  if (mediaKind(entry)) return true;
...
// mediaKind returns "pdf" | "audio" | "video" for files the browser can
// render itself from a decrypted blob (its PDF viewer in a frame; the
// <audio>/<video> elements), or null. Whole-file decrypt, so the preview
// cap applies; streaming decrypt through Range is a later step.
function mediaKind(entry) {
  const mime = (entry.entry?.mime || "").toLowerCase();
  const ext = (entry.name || "").toLowerCase().split(".").pop();
  if (mime === "application/pdf" || ext === "pdf") return "pdf";
  if (mime.startsWith("audio/") || ["mp3","wav","flac","ogg","oga","aac","m4a","opus"].includes(ext)) return "audio";
...
export class FolderUI {
  constructor(root, session, opts = {}) {
    ...
    this.view = "all"; // "all" | "recent" | "photos" | "trash" | "devices" | "backup"
    this.currentDir = "/";
    this.busy = false;
    this.message = null;
    ...
    // Search inside files: the per-device index (lib/search.js), loaded
    // lazily; null until built. `indexing` holds progress while building.
  ...
  static mount(root, session, opts = {}) {
    ...
    ui._wireShell();
    ui.render();
    // Expired trash is swept in the background after the first paint.
    setTimeout(() => { void ui.sweepTrash(); }, 1500);
    void ui._loadSearchIndex();
    return ui;
  }
  ...
  _wireShell() {
    ...
    shell.onSearch((q) => {
      ...
      if (!["all", "recent", "photos", "trash"].includes(this.view)) {
        this.view = "all";
        this.render();
        return;
      }
  ...
  setView(view) {
    if (!["all", "recent", "photos", "trash", "devices", "backup"].includes(view)) view = "all";
    this.view = view;
    this.searchQuery = "";
  ...
  // _offlineGuard: with no network the folder is read-only (listing from
  // the last copy this device saw). Returns true when the action must stop.
  _offlineGuard() {
    if (!this.session?.offline) return false;
    this.setMessage("error", "You're offline — this is the folder as last seen. Reconnect and reload to make changes.");
    return true;
  }
  ...
  render() {
    ...
    if (this.session.offline) {
      const retry = h("button", { type: "button", class: "btn-link" }, ["Reload"]);
      retry.addEventListener("click", () => location.reload());
      this.root.appendChild(h("div", { class: "banner banner-offline", role: "status" }, [
        "Offline — showing this folder as it was last seen on this device. Downloads and changes need a connection. ", retry,
      ]));
    }
  ...
  _syncShell() {
    ...
    const trash = this.session.manifest.materialiseTrash();
    this.shell.setActive(this.view);
    this.shell.setCounts({ photos: stats.photoCount, trash: trash.size });
    this.shell.setStats(stats.fileCount === 0 && trash.size === 0
      ? "Nothing stored yet"
      : `${formatBytes(stats.totalBytes)} in ${stats.fileCount} ${stats.fileCount === 1 ? "file" : "files"} · encrypted`);
  }

  static VIEW_TITLES = { all: "All files", recent: "Recent", photos: "Photos", trash: "Trash", devices: "Devices", backup: "Backup" };
  ...
  buildHeader() {
    ...
    if (this.view === "trash") {
      const emptyBtn = h("button", { type: "button", class: "btn btn-danger-secondary" }, [icon("trash", { size: 16 }), "Empty trash"]);
      if (this.session.manifest.materialiseTrash().size === 0) emptyBtn.disabled = true;
      emptyBtn.addEventListener("click", () => this.handleEmptyTrash());
      titleRow.appendChild(h("div", { class: "folder-toolbar" }, [emptyBtn]));
    }
    if (this.view === "all") {
  ...
  async handleBulkDelete() {
    ...
    try {
      ...
      await this.flushManifest();
      this.clearSelection(false);
      this.setMessage("info", `Moved ${done} files to Trash`);
  ...
  // handleShare mints a per-file link: an expiring presigned URL for the
  // object plus the file's own data key in the URL fragment. A carrier
  // that predates share links (no `share` in its health record) is told
  // how to update rather than handed a link that will 401.
  async handleShare(entry) {
    if (this._offlineGuard()) return;
    if (this.busy) { this.setMessage("error", "Another operation is in flight"); return; }
    if (!entry.entry?.uuid) return;
    const s = this.session;
    if (s.region === "carrier" && s.carrierShare === undefined) {
      try {
        const base = s.bucketBase.replace(/\/o\/$/, "");
        const res = await fetch(base + "/health", { cache: "no-store" });
        const rec = res.ok ? await res.json() : null;
        s.carrierShare = !!rec?.share;
      } catch { s.carrierShare = false; }
    }
    if (s.region === "carrier" && !s.carrierShare) {
      await formDialog({
        title: "Your carrier needs an update",
        description: "Share links need a route this Worker doesn't have yet. On GitHub, open your copy of crate-carrier and click Sync fork (or pull upstream main); Cloudflare redeploys it in about a minute. Then try again.",
        fields: [], confirmLabel: "OK",
      });
      return;
    }
    const answer = await formDialog({
      title: `Share “${entry.name}”`,
      description: "Anyone with the link can download this one file until it expires. The link carries the file's own key, so nothing else in your folder is reachable — and it will sit in browser history like any URL.",
      fields: [{ key: "ttl", label: "Link works for", type: "select", options: SHARE_TTL.map((t) => ({ value: String(t.seconds), label: t.label })), value: String(SHARE_TTL[1].seconds), autofocus: true }],
      confirmLabel: "Make link",
    });
    if (!answer) return;
    const seconds = Number(answer.ttl) || SHARE_TTL[1].seconds;
    this.busy = true;
    let link, exp;
    try {
      ({ link, exp } = await mintShare({ session: s, entry: entry.entry, name: entry.name, seconds, origin: location.origin }));
    } catch (e) {
      console.error(e);
      this.setMessage("error", `Couldn't make a link: ${e.message ?? e}`);
      this.busy = false;
      return;
    }
    this.busy = false;
    const until = new Date(exp).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" });
    await formDialog({
      title: "Share link",
      description: `Works until ${until}. To cut it off earlier, rotate your storage credentials (carrier secret or R2 token) or Delete forever.`,
      fields: [{ key: "link", label: "Link", value: link, mono: true, multiline: true, rows: 4, readonly: true, copy: true }],
      confirmLabel: "Done", cancelLabel: null,
    });
  }

  // --- trash ------------------------------------------------------------

  // handleRestore puts a trashed file back — a `create` with the same uuid
  // and keys, under its old path or "name (restored).ext" if that is taken.
  // The object must still exist: a daemon-side delete removes bytes at
  // once, in which case the entry is purged with a clear message.
  async handleRestore(entry) {
    if (this._offlineGuard()) return;
    if (this.busy) { this.setMessage("error", "Another operation is in flight"); return; }
    const t = entry.entry;
    this.busy = true;
    try {
      const head = await bucket.signedHead({
        url: this.session.bucketBase + OBJECTS_PREFIX + t.uuid,
        region: this.session.region, accessKey: this.session.accessKey, secretKey: this.session.secretKey,
      });
      if (!head.ok && head.status === 404) {
        await this.session.manifest.append(purgeEvent({ uuid: t.uuid }), this.session.masterKey);
        await this.flushManifest();
        this.setMessage("error", `${entry.name} can't be restored — its data was already removed by another device.`);
        return;
      }
      if (!head.ok) throw new Error(`HEAD object failed: ${head.status} ${head.message}`);
      const tree = this.session.manifest.materialise();
      let path = t.path;
      if (tree.has(path)) {
        ...
        let n = 1;
        do { path = `${dir}${stem} (restored${n > 1 ? " " + n : ""})${ext}`; n += 1; } while (tree.has(path));
      }
      await this.session.manifest.append(restoreEvent(t, path), this.session.masterKey);
      await this.flushManifest();
      this.setMessage("info", `Restored ${basename(path)} to ${parentLabel(path)}`);
      this._fireChange({ op: "create", source: "local" });
    } catch (e) {
      console.error(e);
      this.setMessage("error", `Restore failed: ${e.message ?? e}`);
    } finally {
      this.busy = false;
    }
  }

  // handlePurge deletes one trashed file's bytes now.
  async handlePurge(entry) {
    if (this._offlineGuard()) return;
    if (this.busy) { this.setMessage("error", "Another operation is in flight"); return; }
    const modalRoot = document.createElement("div");
    modalRoot.className = "delete-modal";
    document.body.appendChild(modalRoot);
    let ok = false;
    try { ok = await renderDeleteConfirmModal(modalRoot, { isDir: false, forever: true, name: entry.name, path: entry.path }); }
    finally { document.body.removeChild(modalRoot); }
    if (!ok) return;
    this.busy = true;
    try {
      await this.purgeEntries([entry.entry]);
      this.setMessage("info", `Deleted ${entry.name} forever`);
    } catch (e) {
      console.error(e);
  ...
  async handleEmptyTrash() {
    if (this._offlineGuard()) return;
    if (this.busy) { this.setMessage("error", "Another operation is in flight"); return; }
    const all = [...this.session.manifest.materialiseTrash().values()];
    ...
    try {
      ok = await renderDeleteConfirmModal(modalRoot, {
        isDir: false, forever: true, count: all.length, name: `${all.length} files`,
        path: all.slice(0, 6).map((t) => basename(t.path)).join(", ") + (all.length > 6 ? `, … (${all.length - 6} more)` : ""),
      });
    } finally { document.body.removeChild(modalRoot); }
    if (!ok) return;
    this.busy = true;
    try {
      const n = await this.purgeEntries(all);
      this.setMessage("info", `Emptied the trash — ${n} ${n === 1 ? "file" : "files"} removed from storage`);
    } catch (e) {
      console.error(e);
      this.setMessage("error", `Empty trash stopped: ${e.message ?? e}`);
    } finally {
      this.busy = false;
    }
  }

  // purgeEntries deletes each object (404 = already gone) and appends a
  // purge event; one flush at the end (and one on failure, so the purges
  // that landed are recorded). Returns the count purged.
  async purgeEntries(trashed) {
    let n = 0;
    try {
      for (const t of trashed) {
        await this.deleteObject(t.uuid);
        await this.session.manifest.append(purgeEvent({ uuid: t.uuid }), this.session.masterKey);
        n += 1;
      }
    } finally {
      if (n > 0) {
        await this.flushManifest();
        this._fireChange({ op: "purge", source: "local" });
      }
    }
    return n;
  }

  // sweepTrash purges entries older than TRASH_RETENTION_MS. Runs after
  // mount; quiet unless something was removed.
  async sweepTrash() {
    if (this.busy || !this.session?.manifest || this.session.offline) return;
    const cutoff = Date.now() - TRASH_RETENTION_MS;
    const expired = [...this.session.manifest.materialiseTrash().values()].filter((t) => (t.deleted_ts || 0) < cutoff);
    if (expired.length === 0) return;
    this.busy = true;
    try {
      const n = await this.purgeEntries(expired);
      if (n > 0) this.setMessage("info", `Removed ${n} expired ${n === 1 ? "item" : "items"} from Trash`);
    } catch (e) {
      console.warn("trash sweep", e);
    } finally {
      this.busy = false;
    }
  }
  ...
  async deleteObject(uuid) {
    ...
    if (!del.ok && del.status !== 404) throw new Error(`DELETE object failed: ${del.status} ${del.message}`);
  }
  ...
  buildBackupView() {
    ...
    // A v1.1 folder adds the slot without the passphrase (a passkey session
    // has none); a v1.0 folder needs it to migrate.
    if (!this.session.crateJson || (!this.session.passphrase && this.session.crateJson.version !== "1.1")) recBtn.disabled = true;
    ...
    // A v1.1 folder re-wraps under a new passphrase without the old one
    // (a passkey session has none); a v1.0 folder needs it to migrate.
    if (!this.session.crateJson || (!this.session.passphrase && this.session.crateJson.version !== "1.1")) passBtn.disabled = true;
    passBtn.addEventListener("click", () => this.handleChangePassphrase());
    const rekeyBtn = h("button", { type: "button", class: "btn btn-danger-secondary" }, [icon("refresh", { size: 16 }), "Re-key folder"]);
    if (!this.session.crateJson || !this.session.passphrase || this.session.crateJson.version !== "1.1") rekeyBtn.disabled = true;
    if (!this.session.passphrase) rekeyBtn.title = "Re-keying needs the passphrase — unlock with it first.";
    rekeyBtn.addEventListener("click", () => this.handleRekey());
    const gen = this.session.manifest.generation();

    // Passkey on this device. Enrolment state is read lazily; the card
    // renders "checking" first and re-renders once known.
    const pkAvail = passkey.available();
    if (pkAvail && this._passkeyEnrolled === undefined) {
      this._passkeyEnrolled = null;
      passkey.isEnrolled(this.session.bucketBase).then((v) => { this._passkeyEnrolled = v; if (this.view === "backup") this.render(); }).catch(() => { this._passkeyEnrolled = false; });
    }
    const pkOn = this._passkeyEnrolled === true;
    const pkBtn = h("button", { type: "button", class: "btn btn-secondary" }, [icon("key", { size: 16 }), pkOn ? "Remove passkey" : "Set up a passkey"]);
    if (!pkAvail || this._passkeyEnrolled === null) pkBtn.disabled = true;
    pkBtn.addEventListener("click", () => (pkOn ? this.handlePasskeyRemove() : this.handlePasskeyEnrol()));
    ...
    return h("div", { class: "view-cards" }, [
      ...
      h("div", { class: "view-card" }, [
        h("div", { class: "view-card-body" }, [
          h("h2", {}, ["Passkey on this device"]),
          h("p", {}, [!pkAvail
            ? "Passkeys aren't available in this browser."
            : pkOn
              ? "Set up. The start screen offers Unlock with passkey on this device — Touch ID, Face ID, Windows Hello or a security key opens the folder without typing the passphrase. Other devices still need the passphrase or the credentials file."
              : "Open this folder on this device with Touch ID, Face ID, Windows Hello or a security key instead of typing the passphrase. Needs a passkey provider that supports the PRF extension (recent Chrome, Safari, Android, iOS, 1Password, Bitwarden). Nothing is written to your storage; losing the device only loses the shortcut."]),
        ]),
        pkBtn,
      ]),
      h("div", { class: "view-card" }, [
        h("div", { class: "view-card-body" }, [
          h("h2", {}, ["Re-key"]),
          h("p", {}, [`Replaces the folder's encryption key: every file key is re-wrapped under a fresh one and the manifest re-signed, files untouched. For when you suspect the key leaked, or for a folder migrated from v1.0 whose old passphrase you want fully retired. Every other device reopens with the passphrase; a paired crate-agent needs v1.4 or re-pairing; the recovery phrase is replaced by a new one.${gen ? ` Re-keyed ${gen} ${gen === 1 ? "time" : "times"}.` : ""}`]),
        ]),
        rekeyBtn,
      ]),
      h("div", { class: "view-card" }, [
  ...
  async _writeKeySlots({ passphrase = null, recoveryEntropy = null } = {}) {
    ...
    void offline.remember(s.bucketBase, { crateJson: r.bytes });
    return r;
  }
  ...
  async handlePasskeyEnrol() {
    if (this._offlineGuard()) return;
    if (this.busy) return;
    const s = this.session;
    const label = s.bucket?.name || "your folder";
    this.busy = true;
    try {
      await passkey.enrol(s, label);
      this._passkeyEnrolled = true;
      this.setMessage("info", "Passkey set up. The start screen on this device now offers Unlock with passkey.");
    } catch (e) {
      console.error(e);
      this.setMessage("error", e?.name === "NotAllowedError" ? "Passkey setup cancelled." : `Couldn't set up a passkey: ${e.message ?? e}`);
    } finally {
      this.busy = false;
    }
  }

  async handlePasskeyRemove() {
    await passkey.forget(this.session.bucketBase);
    this._passkeyEnrolled = false;
    this.setMessage("info", "Passkey removed from this device. The passkey itself stays in your provider until you delete it there.");
  }

  // _resealPasskey keeps the device record valid after the content key
  // changed (re-key). One platform prompt; on failure the record is
  // dropped so a stale one never lies on the start screen.
  async _resealPasskey() {
    if (this._passkeyEnrolled !== true && !(await passkey.isEnrolled(this.session.bucketBase))) return;
    try {
      await passkey.reseal(this.session);
      this.setMessage("info", "Passkey updated for the new key.");
    } catch (e) {
      console.warn("passkey reseal", e);
      await passkey.forget(this.session.bucketBase);
      this._passkeyEnrolled = false;
      this.setMessage("error", "The passkey on this device couldn't be updated and was removed — set it up again from Backup.");
    }
  }

  // handleRekey runs lib/rekey.js on this session and then walks the user
  // straight into a new recovery phrase (the old slot cannot survive).
  async handleRekey() {
    if (this._offlineGuard()) return;
    if (this.busy) { this.setMessage("error", "Another operation is in flight"); return; }
    const s = this.session;
    const files = this.computeStats().fileCount;
    const answer = await formDialog({
      title: "Re-key this folder?",
      description: `A fresh key for ${files} ${files === 1 ? "file" : "files"}. Nothing is re-uploaded. Afterwards: other devices reopen with your passphrase; crate-agent older than v1.4 must be re-paired; your recovery phrase stops working and you will be shown a new one right away.`,
      fields: [{ key: "confirm", label: "Type re-key to continue", autofocus: true, autocomplete: "off" }],
      confirmLabel: "Re-key now", danger: true,
      validate: (v) => (v.confirm.trim().toLowerCase() === "re-key" ? null : "Type re-key to confirm."),
    });
    if (!answer) return;
    this.busy = true;
    this.setMessage("info", "Re-keying — re-wrapping file keys and re-signing the manifest…");
    let r;
    try {
      r = await rekeyFolder({
        bucketBase: s.bucketBase, region: s.region, accessKey: s.accessKey, secretKey: s.secretKey,
        masterKey: s.masterKey, manifest: s.manifest, manifestETag: s.manifestETag,
        crateJson: s.crateJson, crateJsonETag: s.crateJsonETag, passphrase: s.passphrase,
      });
    } catch (e) {
      console.error(e);
      this.busy = false;
      this.setMessage("error", `Re-key failed: ${e.message ?? e}`);
      return;
    }
    // Swap everything that names the old key in one go; the sync facade
    // reads these through the session, so its next poll uses the new key.
    s.masterKey = r.masterKey;
    s.manifest = r.manifest;
    s.manifestETag = r.manifestETag;
    s.lastFlushedEventCount = r.manifest.events.length;
    s.crateJson = r.crateJson;
    s.crateJsonETag = r.crateJsonETag;
    s.salt = r.crateJson.passphraseWrap.saltBytes;
    s.hasRecovery = false;
    this.searchIndex = null; // was encrypted under the old key
    void search.clearIndex(s).catch(() => {});
    this.busy = false;
    this.setMessage("info", `Re-keyed (generation ${r.manifest.generation()}). Now write down your new recovery phrase.`);
    this._fireChange({ op: "rekey", source: "local" });
    await this.handleEnableRecovery();
    await this._resealPasskey();
  }
  ...
  buildTree() {
    ...
    if (entries.length === 0 && !(q && this.view === "all")) {
      wrap.appendChild(this.buildEmptyState());
      return wrap;
    }
    if (entries.length === 0) wrap.appendChild(this.buildEmptyState());
    ...
    if (q && this.view === "all") this.appendContentHits(wrap, q, new Set(entries.map((e) => e.path)));
    return wrap;
  }
  ...
  async _loadSearchIndex() {
    if (!this.session?.manifest || this.session.offline) { this.searchIndex = null; return; }
    try { this.searchIndex = await search.loadIndex(this.session); } catch { this.searchIndex = null; }
    if (this.searchQuery.trim() && this.view === "all") {
      const treeRoot = this.root.querySelector(".folder-tree");
      if (treeRoot) treeRoot.replaceWith(this.buildTree());
    }
  }

  // appendContentHits adds a "In file contents" section under the name
  // matches: hits from the index, or the offer to build/refresh it.
  appendContentHits(wrap, q, shownPaths) {
    const tree = this.session.manifest.materialise();
    const idx = this.searchIndex;
    const stale = search.staleCount(idx || null, this.session.manifest);
    const total = search.candidates(this.session.manifest).length;
    const head = (label, action) => h("li", { class: "folder-section" }, [h("span", {}, [label]), action || null]);
    if (this.indexing) {
      wrap.appendChild(head(`Indexing text files… ${this.indexing.done} of ${this.indexing.total}`));
      return;
    }
    if (!idx) {
      if (total === 0) return;
      const btn = h("button", { type: "button", class: "btn-link" }, [`Search inside ${total} text ${total === 1 ? "file" : "files"}`]);
      btn.addEventListener("click", () => this.handleBuildIndex());
      wrap.appendChild(head("In file contents", btn));
      wrap.appendChild(h("li", { class: "folder-empty muted small" }, ["Not indexed yet. Indexing reads your text files in this tab and keeps an encrypted index on this device only."]));
      return;
    }
    const hits = search.queryIndex(idx, q).filter((hit) => tree.has(hit.path) && !shownPaths.has(hit.path));
    const refresh = stale > 0 ? h("button", { type: "button", class: "btn-link" }, [`Refresh (${stale} changed)`]) : null;
    if (refresh) refresh.addEventListener("click", () => this.handleBuildIndex());
    wrap.appendChild(head(hits.length ? `In file contents · ${hits.length}` : "In file contents · none", refresh));
    for (const hit of hits.slice(0, 50)) {
      const entry = tree.get(hit.path);
      const row = { name: basename(hit.path), isDir: false, entry, path: hit.path, sub: hit.snippet || parentLabel(hit.path) };
      wrap.appendChild(this.buildTreeRow(row));
    }
  }
  ...
  async handleBuildIndex() {
    ...
    this.indexing = { done: 0, total: search.staleCount(this.searchIndex || null, this.session.manifest) };
    const refreshTree = () => {
      const treeRoot = this.root.querySelector(".folder-tree");
      if (treeRoot) treeRoot.replaceWith(this.buildTree());
    };
    refreshTree();
    try {
      this.searchIndex = await search.buildIndex(this.session, {
        previous: this.searchIndex || null,
        onProgress: (p) => {
          this.indexing = { done: p.done, total: p.total };
          const sec = this.root.querySelector(".folder-section span");
          if (sec) sec.textContent = `Indexing text files… ${p.done} of ${p.total}`;
        },
      });
      this.setMessage("info", `Indexed ${Object.keys(this.searchIndex.files).length} text files for search on this device.`);
    } catch (e) {
      console.error(e);
      this.setMessage("error", `Indexing stopped: ${e.message ?? e}`);
    } finally {
      this.indexing = null;
      refreshTree();
    }
  ...
  buildEmptyState() {
    ...
    if (this.view === "trash") {
      return h("li", { class: "folder-empty" }, [
        h("div", { class: "empty-icon", "aria-hidden": "true" }, [icon("trash", { size: 28 })]),
        h("p", { class: "empty-title" }, ["Trash is empty."]),
        h("p", { class: "muted small" }, ["Deleted files stay here for 30 days, then their data is removed from your storage."]),
      ]);
    }
    if (this.view === "photos") {
      return h("li", { class: "folder-empty" }, [
  ...
  entriesForView(view) {
    if (view === "trash") {
      const out = [];
      for (const [, t] of this.session.manifest.materialiseTrash()) {
        out.push({ name: basename(t.path), isDir: false, entry: t, path: t.path, trashed: true, sub: `was in ${parentLabel(t.path)} · deleted ${shortDate(t.deleted_ts)}` });
      }
      out.sort((a, b) => (b.entry.deleted_ts || 0) - (a.entry.deleted_ts || 0));
      return out;
    }
    const tree = this.session.manifest.materialise();
    const out = [];
  ...
  buildTreeRow(entry) {
    ...
    if (entry.trashed) {
      actions.appendChild(action("refresh", "Restore", () => this.handleRestore(entry)));
      actions.appendChild(action("x", "Delete forever", () => this.handlePurge(entry), " btn-danger"));
    } else {
      ...
      if (!entry.isDir) actions.appendChild(action("link", "Share link", () => this.handleShare(entry)));
    ...
    const activate = () => {
      if (entry.trashed) {
        if (previewable(entry)) this.handlePreview(entry);
        return;
      }
      if (entry.isDir) {
  ...
  async uploadOne(file) {
    ...
    // sealFile deflates compressible files first (lib/crypto.js).
    const sealed = await cryptoLib.sealFile(dataKey, bytes, uuid, { name: file.name, mime: file.type });
  ...
  async handleDelete(entry) {
    ...
    this.busy = true;
    try {
      // Soft delete: the object stays in the bucket for TRASH_RETENTION_MS
      // and the file can be restored from Trash; only the manifest changes.
      ...
      await this.flushManifest();
      this.setMessage("info", `Moved ${entry.name} to Trash`);
      this._fireChange({ op: "delete", source: "local" });
    } catch (e) {
      console.error(e);
      this.setMessage("error", `Delete failed: ${e.message ?? e}`);
...
function renderDeleteConfirmModal(modalRoot, entry) {
  return new Promise((resolve) => {
    ...
    const forever = !!entry.forever;
    card.appendChild(h("h2", { id: "delete-dialog-title" }, [
      forever
        ? (many ? `Delete ${entry.count} files forever?` : "Delete forever?")
        : (many ? `Move ${entry.count} files to Trash?` : entry.isDir ? "Delete folder?" : "Move to Trash?"),
    ]));
    card.appendChild(h("p", {
      ...
      forever
        ? "The encrypted data is removed from your storage now. This can't be undone."
        : entry.isDir
          ? "This empty folder will be removed from your Crate. This can't be undone."
          : "It stays in Trash for 30 days, where you can restore it; after that its data is removed from your storage.",
    ]));
    ...
    const deleteBtn = h("button", {
      ...
      ? (many ? `Delete ${entry.count} files forever` : "Delete forever")
      : (many ? `Move ${entry.count} to Trash` : entry.isDir ? "Delete folder" : "Move to Trash")]);
...
async function renderPreviewModal(modalRoot, entry, bytes) {
  return new Promise((resolve) => {
    ...
    } else if (mediaKind(entry) === "pdf") {
      blobUrl = URL.createObjectURL(new Blob([bytes], { type: "application/pdf" }));
      // The browser's own PDF viewer, in a frame. CSP frame-src allows blob:.
      card.appendChild(h("iframe", { src: blobUrl, class: "preview-frame", title: entry.name }));
      card.appendChild(h("p", { class: "muted small" }, ["Rendered by your browser's PDF viewer from the decrypted bytes in this tab."]));
    } else if (mediaKind(entry) === "audio" || mediaKind(entry) === "video") {
      const kind = mediaKind(entry);
      const ext = (entry.name || "").toLowerCase().split(".").pop();
      const guess = { mp3: "audio/mpeg", wav: "audio/wav", flac: "audio/flac", ogg: "audio/ogg", oga: "audio/ogg", aac: "audio/aac", m4a: "audio/mp4", opus: "audio/ogg", mp4: "video/mp4", m4v: "video/mp4", webm: "video/webm", mov: "video/quicktime", ogv: "video/ogg" }[ext];
      const mime = (entry.entry?.mime && entry.entry.mime !== "application/octet-stream") ? entry.entry.mime : (guess || `${kind}/*`);
      blobUrl = URL.createObjectURL(new Blob([bytes], { type: mime }));
      const el = h(kind, { src: blobUrl, controls: "controls", class: kind === "video" ? "preview-video" : "preview-audio", preload: "metadata" });
      el.addEventListener("error", () => {
        el.replaceWith(h("p", { class: "muted" }, [`Your browser can't play this ${kind} format (${mime}). Download it to open in a player.`]));
      });
      card.appendChild(el);
    } else {
