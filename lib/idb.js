...
// wizard state minus secrets — passphrase and recovery phrase never touch
// disk, per crate-browser-handoff §"Persistence rules").
//
// Reused from M1 onward; the M2+ session cache binds to the same DB
// with additional stores added in `openDB()`'s upgrade callback.

const DB_NAME = "crate-session";
const DB_VERSION = 1;
const STORES = ["prefs", "onboarding"];
