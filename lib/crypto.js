...
// match the daemon's wire format so the two surfaces interoperate byte-for-
// byte once the salt is shared via .crate/crate.json (per crate-daemon-
// handoff-v1.0.md + the daemon's internal/cratejson + internal/kdf).
//
// Spec: crate-browser-handoff-v1.0.md §"Encryption details" + §"Manifest
// format".
//
// Decisions locked at M3 (per plan/pending.md §M3 open questions):
//   - PBKDF2 iterations: 600,000 (spec mandate)
//   - Master key:    deriveBits → extractable Uint8Array. Hygiene
//                    mitigated by explicit zeroing. We need the raw bytes
//                    for HMAC-SHA256 signing of manifest events.
//   - Payload AEAD:  AES-256-GCM. Per-file random 12-byte IV.
//   - Key wrapping:  AES-256-GCM (NOT AES-KW). One less primitive, the
//                    fresh nonce per wrap is fine for our key-wrap volumes,
//                    and SubtleCrypto's wrapKey adds complexity we don't
//                    need (we treat data keys as raw bytes throughout).
//   - Manifest sig:  HMAC-SHA256(masterKey, canonical_event_json). Each
//                    event includes prev_sig — tamper-evident chain.
