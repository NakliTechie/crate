...
export class Crate {
  ...
  static async open({ bucketConfig, credentials, passphrase } = {}) {
    ...
    // v1.0 (legacy): master key = PBKDF2(passphrase, salt).
    // v1.1: master key is random ("content key"), wrapped under
    //       passphrase-KEK = PBKDF2(passphrase, passphrase_wrap.salt).
    //       Unwrap to get the same 32-byte key downstream code expects.
    let masterKey, salt;
    if (cj.version === "1.1") {
      const pw = cj.passphraseWrap;
      salt = pw.saltBytes;
      const kek = await cryptoLib.derivePassphraseKEK(passphrase, pw.saltBytes, pw.iter);
      try {
        masterKey = await cryptoLib.unwrapKey(kek, pw.ivBytes, pw.ctBytes);
      } catch (e) {
        cryptoLib.zero(kek);
        throw new CrateError("open: wrong passphrase (passphrase_wrap unwrap failed)");
      }
      cryptoLib.zero(kek);
    } else {
      salt = cj.saltBytes;
      masterKey = await cryptoLib.deriveMasterKey(passphrase, salt);
    }
