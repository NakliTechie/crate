...
// Bahi, Mahalla, …) bind to. Locked at M5 per the milestone schedule.
//
...
export class Crate {
  constructor({ bucketBase, region, accessKey, secretKey, masterKey, manifest, salt }) {
  ...
  async _flushManifest() {
    ...
    if (!put.ok) throw new CrateError(`_flushManifest: PUT failed (${put.status})`);
  }
}
