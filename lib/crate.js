  ...
  async write(path, bytes, { mime } = {}) {
    ...
    if (existing && !existing.isDir && existing.uuid) {
      ...
      await this._manifest.append(
        updateEvent({ uuid: existing.uuid, size: bytes.length, contentIv: sealed.contentIv, chunkSize: sealed.chunkSize, compression: sealed.compression, storedSize: sealed.storedSize }),
        this._masterKey,
      );
