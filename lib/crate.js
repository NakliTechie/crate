...
import { sealVault, openVault, rewrapVault, VaultError } from "./vault.js";
...
export class Crate {
  ...
  async _rewrite({ passphrase = null, recoveryEntropy = null } = {}) {
    this._guardOpen();
    if (!this._crateJson || this._crateJson.version !== "1.1") {
      throw new CrateError("this folder is a v1.0 vault; enable recovery migrates it first");
    }
    let bytes;
    try {
      bytes = await rewrapVault(this._crateJson, this._masterKey, { passphrase, recoveryEntropy });
    } catch (e) {
      if (e instanceof VaultError) throw new CrateError(e.message, { cause: e });
      throw e;
    }
    const put = await bucket.signedPut({
      url: this._bucketBase + cratejson.CRATE_PATH,
      body: bytes, contentType: "application/json",
      ifMatch: this._crateJsonETag || undefined,
      region: this._region, accessKey: this._accessKey, secretKey: this._secretKey,
    });
    if (!put.ok) {
      throw new CrateError(put.status === 412
        ? "the folder's key file changed on another device — reopen and try again"
        : `write .crate/crate.json failed (${put.status} ${put.code}: ${put.message})`);
    }
    this._crateJson = cratejson.parse(bytes);
    this._crateJsonETag = put.etag || null;
    this._salt = this._crateJson.passphraseWrap.saltBytes;
    this._hasRecovery = !!this._crateJson.recoveryWrap;
  }
  ...
  // enableRecovery adds (or replaces) the recovery slot on a v1.1 vault.
  async enableRecovery(recoveryEntropy) {
    ...
    await this._rewrite({ recoveryEntropy });
  }
