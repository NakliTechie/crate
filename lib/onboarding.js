...
import { estimate, whenReady, MIN_BITS, HARD_MIN_BITS } from "./entropy.js";
...
import { entropyToMnemonic, mnemonicToEntropy } from "./recovery.js";
...
export const STAGES = Object.freeze([
  ...
  "cors",
  "passphrase",
  "recovery", // 24-word recovery phrase: second key slot in crate.json (v1.1)
  "done",
...
const CARRIER_STAGES = ["welcome", "carrier", "passphrase", "recovery", "done"];
...
export function createWizard({ root, onComplete, liveRegion } = {}) {
  ...
  const state = {
    ...
    data: {
      ...
      recovery: { words: null, entropy: null, confirmed: [], skipped: false }, // memory-only
      unlock: {
  ...
  // The passphrase stage's inputs call updateNav(); when the same stage is
  // embedded elsewhere (set a new passphrase after a recovery unlock), the
  // host registers a listener here to follow `passphrase.confirmed`.
  let passphraseListener = null;
  function notifyPassphrase() {
    updateNav();
    if (passphraseListener) { try { passphraseListener(); } catch (e) { console.error(e); } }
  }
  ...
  function canAdvance(stage) {
    switch (stage) {
      ...
      case "recovery": return state.data.recovery.skipped || state.data.recovery.confirmed.length >= 3;
  ...
  function reset() {
    ...
    // A previous run's outcome (done or failed) must not short-circuit
    // the next Done stage in this tab.
    ...
    state.data.recovery = { words: null, entropy: null, confirmed: [], skipped: false };
    state.data.unlock = {
      mode: "file",
      fileBytes: null, fileHint: null, fileFromSession: false,
      bucketName: "", accountId: "",
      ...
      manualKind: "carrier", carrierUrl: "",
      passphrase: "",
      credential: "passphrase", phrase: "", recovered: null,
      status: "idle", error: null,
    };
  ...
  // unlockWithPasskey: the platform prompt yields the device-local record
  // (connection details + content key); the folder opens without a
  // passphrase. Errors land on the landing card.
  async function unlockWithPasskey(btn) {
    btn.disabled = true;
    const note = el("p", { class: "muted small", role: "status" }, ["Waiting for your passkey…"]);
    btn.insertAdjacentElement("afterend", note);
    try {
      const r = await passkey.unlock();
      note.textContent = "Opening your folder…";
      const { Crate } = await import("./crate.js");
      const b = r.creds.bucket;
      const bucketConfig = { provider: r.creds.provider, accountId: b.accountId, name: b.name, region: b.region || "auto", url: b.url };
      const credentials = { accessKey: r.creds.credentials.accessKey, secretKey: r.creds.credentials.secretKey };
      let crate;
      try {
        crate = await Crate.open({ bucketConfig, credentials, contentKey: r.masterKey });
      } finally {
        cryptoLib.zero(r.masterKey);
      }
      state.route = "unlock";
      await openSession(crate, bucketConfig, credentials, null);
    } catch (e) {
      console.error(e);
      note.textContent = e?.name === "NotAllowedError" ? "Passkey prompt cancelled." : (e?.message ?? String(e));
      btn.disabled = false;
    }
  }
  ...
  function renderWelcome() {
    ...
    // Passkey shortcut: shown once the enrolled list is known (async).
    const actions = el("div", { class: "landing-actions" }, [startBtn, unlockBtn]);
    if (passkey.available()) {
      passkey.listEnrolled().then((list) => {
        if (!list.length || !actions.isConnected) return;
        const label = list.length === 1 ? `Unlock with passkey · ${list[0].label}` : "Unlock with passkey";
        const pkBtn = el("button", { type: "button", class: "btn btn-primary btn-block btn-lg" }, [label]);
        pkBtn.addEventListener("click", () => unlockWithPasskey(pkBtn));
        startBtn.classList.replace("btn-primary", "btn-secondary");
        startBtn.classList.remove("btn-lg");
        actions.insertBefore(pkBtn, startBtn);
      }).catch(() => {});
    }
  ...
  function renderSuggestedPassphrase() {
    ...
    return makeFragment(
      el("section", { class: "stage stage-passphrase" }, [
        ...
        el("p", { class: "muted" }, [
          "These five words, joined with hyphens, are your passphrase. Write them down now — on paper, or in your password manager. Nobody can recover them for you, and Crate never stores them.",
        ]),
        chips,
        el("p", { class: "pp-typed" }, ["Type it exactly like this, hyphens included:"]),
        el("code", { class: "code-block pp-typed-value", tabindex: "0", "aria-label": "Your passphrase, typed out" }, [pp.suggested]),
        el("p", { class: "muted small" }, ["In Crate's own unlock screen, spaces between the words work too; the desktop daemon wants the hyphens."]),
        el("div", { class: "row" }, [copyBtn, againBtn]),
        el("label", { class: "ack", for: "pp-saved" }, [box, el("span", {}, ["I've saved the hyphenated passphrase somewhere I'll find it again."])]),
  ...
  // Recovery phrase: 24 BIP-39 words encoding 32 random bytes; the content
  // key is wrapped under PBKDF2(those bytes) as a second slot in
  // crate.json, so a lost passphrase is survivable. Confirming three words
  // is what makes "I wrote it down" true. Skipping is allowed — the vault
  // is then passphrase-only until the user enables recovery from Backup.
  function renderRecovery() {
    const rec = state.data.recovery;
    if (!rec.words) {
      rec.entropy = cryptoLib.randomBytes(32);
      rec.words = []; // sentinel while the checksum computes
      rec.confirmed = [];
      rec.ask = null;
      entropyToMnemonic(rec.entropy).then((w) => {
        rec.words = w;
        if (state.stage === "recovery") render();
      }).catch((e) => {
        console.error("recovery: phrase generation failed", e);
        rec.words = null;
        rec.entropy = null;
      });
    }
    if (!Array.isArray(rec.words) || rec.words.length !== 24) {
      return makeFragment(el("section", { class: "stage stage-recovery" }, [
        el("h2", { id: "stage-title" }, ["Preparing your recovery phrase…"]),
      ]));
    }
    const words = rec.words;
    const grid = el("ol", { class: "recovery-grid", "aria-label": "Recovery phrase, 24 words" },
      words.map((w, i) => el("li", { class: "recovery-word" }, [
        el("span", { class: "recovery-index", "aria-hidden": "true" }, [String(i + 1)]),
        el("span", { class: "recovery-text" }, [w]),
      ])));
    const copyBtn = makeCopyButton(() => words.join(" "), "Copy phrase");
    const againBtn = el("button", { type: "button", class: "btn btn-secondary" }, ["Different words"]);
    againBtn.addEventListener("click", () => {
      rec.words = null; rec.entropy = null; rec.confirmed = []; rec.skipped = false; rec.ask = null;
      render();
      announce(liveRegion, "New recovery phrase shown.");
    });

    // Three positions to type back, fixed for this phrase so Back/Next
    ...
    if (!rec.ask) {
      ...
      rec.ask = all.slice(0, 3).sort((a, b) => a - b);
    }
    const correct = new Set(rec.confirmed);
    const fields = el("div", { class: "field-row" }, rec.ask.map((idx) => {
      const inp = el("input", {
        type: "text", class: "input input-narrow", autocomplete: "off", autocapitalize: "none", spellcheck: "false",
        "aria-label": `Word ${idx + 1}`, placeholder: `word ${idx + 1}`,
        value: correct.has(idx) ? words[idx] : "",
      });
      if (correct.has(idx)) inp.classList.add("input-ok");
      inp.addEventListener("input", () => {
        const v = inp.value.trim().toLowerCase();
        const ok = v === words[idx];
        inp.classList.toggle("input-ok", ok);
        inp.classList.toggle("input-fail", !ok && v.length > 0);
        if (ok) correct.add(idx); else correct.delete(idx);
        rec.confirmed = [...correct];
        rec.skipped = false;
        updateNav();
      });
      return el("div", { class: "confirm-cell" }, [el("label", { for: `rec-${idx}` }, [`Word ${idx + 1}`]), inp]);
    }));

    const skipLink = el("button", { type: "button", class: "btn-link" }, ["Skip for now — I'll add a recovery phrase later from Backup"]);
    ...
    return makeFragment(
      el("section", { class: "stage stage-recovery" }, [
        el("h2", { id: "stage-title" }, ["Your recovery phrase"]),
        el("p", { class: "muted" }, [
          "If you ever lose your passphrase, these 24 words open the folder and let you set a new one. Write them on paper, in order. Don't screenshot them; don't email them to yourself. Nobody can recover them for you.",
        ]),
        grid,
        el("div", { class: "row" }, [copyBtn, againBtn]),
        el("hr", { class: "stage-divider" }),
        el("p", {}, ["Type three of the words back to confirm you have them:"]),
        fields,
        el("p", { class: "muted small" }, [skipLink]),
      ]),
    );
  }
  ...
  function renderDone() {
    ...
    if (status === "running") {
      ...
      const ackText = el("span", {}, [
        ...
        state.data.recovery?.skipped || !state.data.recovery?.words?.length ? "." : ", and my recovery phrase is on paper.",
      ]);
      ...
      el("section", { class: "stage stage-done" }, [
        ...
        status === "done"
          ? el("div", { class: "hint-card" }, [
              ...
              el("p", { class: "muted small", style: "margin: 0;" }, [
                "Download your credentials file and keep it with your passphrase",
                state.data.recovery?.skipped || !state.data.recovery?.words?.length ? "" : " and the 24-word recovery phrase you wrote down",
                ". On another device you unlock with this file + your passphrase. The file holds your connection details, encrypted — it does not replace your passphrase, and it is useless without it. Store it in ",
                el("a", { href: "https://tijori.naklitechie.com/", target: "_blank", rel: "noopener" }, ["Tijori"]),
  ...
  async function runFirstTimeSetup() {
    ...
    const recoveryEntropy = data.recovery?.skipped ? null : (data.recovery?.entropy || null);

    // Crate.bootstrap writes the v1.1 crate.json (passphrase slot, plus the
    // recovery slot when the phrase was kept) and the empty signed
    // manifest, and saves the rollback anchor — the same code path the
    // ESM API uses, so the wizard cannot drift from it.
    const { Crate } = await import("./crate.js");
    const crate = await Crate.bootstrap({
      bucketConfig, credentials, passphrase, recoveryEntropy,
      createdBy: cratejson.shortBrowserFingerprint(),
    });
    const region = crate._region;
    const bucketBase = crate._bucketBase;
    const masterKey = crate._masterKey;
    const manifest = crate._manifest;
    const salt = crate._salt;
    const putManifest = { etag: crate._manifestETag };
    ...
    const session = {
      ...
      hasRecovery: !!recoveryEntropy,
      crateJson: crate._crateJson,
    };
    // The phrase's entropy has done its job; keep only the words for the
    // Done screen's reminder, and drop those on Start over.
    data.recovery.entropy = null;
  ...
  // openSession turns an open Crate into the session handle the folder
  // UI consumes, stashes the creds for refresh-resume, and hands off.
  async function openSession(crate, bucketConfig, credentials, passphrase) {
    ...
    const session = {
      ...
      salt: crate._salt,
      hasRecovery: !!crate._hasRecovery,
      crateJson: crate._crateJson || null,
      crateJsonETag: crate._crateJsonETag || null,
      manifestETag: crate._manifestETag || null,
      lastFlushedEventCount: crate._manifest.events.length,
      offline: !!crate._offline,
    };
    // Stash the session creds so a refresh keeps the tab "logged in"
    // (one-passphrase unlock instead of full restart). A passkey session
    // has no passphrase to seal the stash under: a reload asks for the
    // passkey again instead.
    if (passphrase) try {
  ...
  function renderUnlock() {
    ...
    // doUnlock opens the folder with a passphrase (trying its dash/space
    // forms) or with the recovery phrase's entropy. A phrase unlock does
    // not open the folder yet: the passphrase is lost, so the user sets a
    // new one first (renderNewPassphrase), which also re-encrypts the
    // creds stash under it.
    async function doUnlock(bucketConfig, credentials, { passphrase = null, recoveryEntropy = null } = {}) {
      ...
      try {
        ...
        if (recoveryEntropy) {
          let crate;
          try {
            crate = await Crate.open({ bucketConfig, credentials, recoveryEntropy });
          } catch (e) {
            if (/wrong recovery phrase/.test(String(e?.message))) {
              throw new Error("That recovery phrase doesn't open this folder — check the words and their order. If the folder was re-keyed, only the phrase shown after the re-key works.", { cause: e });
            }
            if (/no recovery phrase/.test(String(e?.message))) {
              throw new Error("This folder has no recovery phrase set up, so it can only be opened with its passphrase.", { cause: e });
            }
            throw e;
          }
          data.recovered = { crate, bucketConfig, credentials };
        ...
        await openSession(crate, bucketConfig, credentials, passphrase);
      } catch (e) {
        console.error(e);
    ...
    // --- after a recovery-phrase unlock: set a new passphrase ------------
    function renderNewPassphrase() {
      const { crate, bucketConfig, credentials } = data.recovered;
      const wrap = el("div", { class: "stage-unlock-newpass" });
      const embedded = renderPassphrase();
      // The embedded stage carries its own h2#stage-title; this screen's
      // heading owns that id, so demote the inner one.
      const innerTitle = embedded.querySelector("#stage-title");
      if (innerTitle) { innerTitle.removeAttribute("id"); innerTitle.classList.add("small-heading"); }
      wrap.appendChild(embedded);

      const setBtn = el("button", { type: "button", class: "btn btn-primary btn-block" }, ["Set passphrase and open folder"]);
      setBtn.disabled = !state.data.passphrase.confirmed;
      passphraseListener = () => { setBtn.disabled = !state.data.passphrase.confirmed; };
      setBtn.addEventListener("click", async () => {
        const newPass = state.data.passphrase.value;
        if (!newPass) return;
        setBtn.disabled = true;
        data.status = "checking"; data.error = null; pill.check("Saving the new passphrase…");
        try {
          await crate.setPassphrase(newPass);
          passphraseListener = null;
          data.recovered = null;
          data.credential = "passphrase"; data.phrase = "";
          await openSession(crate, bucketConfig, credentials, newPass);
        } catch (e) {
          console.error(e);
          data.status = "fail"; data.error = e.message ?? String(e);
      ...
      wrap.appendChild(el("div", { class: "row" }, [setBtn, pill.el]));
      const cancel = el("button", { type: "button", class: "btn-link" }, ["Cancel — go back without changing anything"]);
      cancel.addEventListener("click", () => {
        try { crate.close(); } catch {}
        passphraseListener = null;
        data.recovered = null; data.status = "idle"; data.error = null;
        render();
      });
      wrap.appendChild(el("p", { class: "muted small" }, [cancel]));
      return wrap;
    }
    ...
    function renderFileMode() {
      ...
      // Fallback links: no file → manual details; no passphrase → the
      // recovery phrase (which needs the details too — the file is
      // encrypted under the passphrase that was lost).
      const manualLink = el("button", { type: "button", class: "btn-link" }, ["No file? Enter the details manually."]);
      manualLink.addEventListener("click", () => { data.mode = "manual"; data.credential = "passphrase"; data.status = "idle"; data.error = null; render(); });
      const lostLink = el("button", { type: "button", class: "btn-link" }, ["Lost your passphrase? Use your recovery phrase."]);
      lostLink.addEventListener("click", () => { data.mode = "manual"; data.credential = "recovery"; data.status = "idle"; data.error = null; render(); });
      wrap.appendChild(el("p", { class: "muted small" }, [manualLink, " · ", lostLink]));
    ...
    function renderManualMode() {
      ...
      // Which credential opens the key: the passphrase (default) or the
      // 24-word recovery phrase written down at setup.
      const recoveryMode = data.credential === "recovery";
      const credRow = el("div", { class: "row" }, [
        el("span", { class: "muted small" }, ["Open with:"]),
        el("button", { type: "button", class: `btn btn-secondary${recoveryMode ? "" : " active"}` }, ["Passphrase"]),
        el("button", { type: "button", class: `btn btn-secondary${recoveryMode ? " active" : ""}` }, ["Recovery phrase"]),
      ]);
      credRow.children[1].addEventListener("click", () => { data.credential = "passphrase"; data.status = "idle"; data.error = null; render(); });
      credRow.children[2].addEventListener("click", () => { data.credential = "recovery"; data.status = "idle"; data.error = null; render(); });
      wrap.appendChild(credRow);

      let phraseState = null; // { entropy } once the typed phrase checks out
      if (recoveryMode) {
        const phraseInput = el("textarea", {
          class: "input mono phrase-input", rows: "4", autocomplete: "off", autocapitalize: "none", spellcheck: "false",
          placeholder: "the 24 words, in order, separated by spaces",
          "aria-label": "Recovery phrase",
        });
        phraseInput.value = data.phrase || "";
        const phraseHelp = el("p", { class: "field-help muted", "aria-live": "polite" }, ["24 words from the recovery step; lowercase, any spacing."]);
        let checkSeq = 0;
        const checkPhrase = async () => {
          data.phrase = phraseInput.value;
          const seq = ++checkSeq;
          const count = data.phrase.trim().split(/\s+/).filter(Boolean).length;
          phraseState = null;
          phraseInput.classList.remove("input-ok", "input-fail");
          if (count === 0) { phraseHelp.textContent = "24 words from the recovery step; lowercase, any spacing."; return; }
          if (count !== 24) { phraseHelp.textContent = `${count} of 24 words.`; return; }
          try {
            const entropy = await mnemonicToEntropy(data.phrase);
            if (seq !== checkSeq) return;
            phraseState = { entropy };
            phraseInput.classList.add("input-ok");
            phraseHelp.textContent = "24 words, checksum OK.";
          } catch (e) {
            if (seq !== checkSeq) return;
            phraseInput.classList.add("input-fail");
            phraseHelp.textContent = String(e.message || e).replace(/^recovery: /, "");
          }
        };
        phraseInput.addEventListener("input", checkPhrase);
        wrap.appendChild(el("div", { class: "field" }, [el("label", {}, ["Recovery phrase"]), phraseInput, phraseHelp]));
        if (data.phrase) void checkPhrase();
      } else {
        wrap.appendChild(field("Folder passphrase", data.passphrase, "password", (v) => data.passphrase = v, "current-password", false));
      }

      const unlockBtn = el("button", { type: "button", class: "btn btn-primary btn-block" }, [recoveryMode ? "Open with recovery phrase" : "Unlock folder"]);
      unlockBtn.addEventListener("click", async () => {
        let credential;
        if (recoveryMode) {
          if (!phraseState) { data.status = "fail"; data.error = "Enter all 24 words of your recovery phrase first."; render(); return; }
          credential = { recoveryEntropy: phraseState.entropy };
        } else {
          if (!data.passphrase) { data.status = "fail"; data.error = "Enter your passphrase."; render(); return; }
          credential = { passphrase: data.passphrase };
        }
        if (carrierMode) {
          const url = (data.carrierUrl || "").trim().replace(/\/+$/, "");
          if (!validCarrierUrl(url) || !data.secretKey) {
            data.status = "fail"; data.error = "Worker URL (https://…) and the carrier secret are both required"; render();
            return;
          }
          unlockBtn.disabled = true;
          await doUnlock(
        ...
        if (!data.bucketName || !data.accountId || !data.accessKey || !data.secretKey) {
          data.status = "fail"; data.error = "All bucket fields are required"; render();
          return;
        }
        unlockBtn.disabled = true;
        await doUnlock(
    ...
    if (data.recovered) {
      return makeFragment(
        el("section", { class: "stage stage-unlock" }, [
          el("h2", { id: "stage-title" }, ["Recovery phrase accepted — set a new passphrase"]),
          el("p", { class: "muted" }, ["Your folder is open. Pick a new passphrase now: it replaces the lost one for this folder on every device, and your recovery phrase keeps working. Nothing in the folder is re-encrypted."]),
          renderNewPassphrase(),
        ]),
      );
    }
    ...
    const intro = data.fileFromSession
      ...
      : data.mode === "manual"
        ? (data.credential === "recovery"
          ? "Enter the connection details from setup, then your 24-word recovery phrase. You'll set a new passphrase right after."
