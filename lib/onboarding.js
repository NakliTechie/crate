...
export function createWizard({ root, onComplete, liveRegion } = {}) {
  ...
  function renderBucket() {
    ...
    verifyBtn.addEventListener("click", async () => {
      ...
      // Shape check first — instant feedback, avoids a wasted request.
      ...
      if (!validBucketName(name)) {
        pill.fail("✗ Bucket name invalid (3–63 chars; lowercase + digits + hyphens)");
        state.data.bucket.verified = false;
        verifyBtn.disabled = false;
        updateNav();
        return;
      ...
      pill.check();
      const url = bucket.endpoints.R2(accountId, name);
      try {
        const r = await bucket.unauthHead({ url, signal: aborter.fresh() });
        if (r.status === 200 || r.status === 401 || r.status === 403) {
          ...
          pill.fail("✗ Bucket not found at this Account ID");
          state.data.bucket.verified = false;
          lastBucketProbeReachable = false;
        } else if (r.networkError) {
          pill.fail("✗ Account ID not resolved");
          state.data.bucket.verified = false;
          lastBucketProbeReachable = false;
        } else {
          pill.fail(`✗ Unexpected status ${r.status}`);
          state.data.bucket.verified = false;
          lastBucketProbeReachable = false;
        }
      } catch (err) {
        if (bucket.isAbortError(err)) { verifyBtn.disabled = false; return; }
    ...
    return makeFragment(
      el("section", { class: "stage stage-bucket" }, [
        ...
        el("p", { class: "muted small" }, ["Verify hits R2 unauthenticated — a 401 from R2 confirms the bucket exists; we'll authenticate at the next step."]),
      ]),
    );
  }
  ...
  function renderCors() {
    ...
    preflightBtn.addEventListener("click", async () => {
      ...
      try {
        const r = await bucket.corsPreflight({
          url, origin: location.origin, signal: aborter.fresh(),
        });
        if (r.ok) {
