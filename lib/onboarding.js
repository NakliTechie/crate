...
// per browser profile.
const HELP_SEEN_KEY = "crate:help-seen-v1";
...
function promiseList() {
  const item = (icon, title, detail) =>
    el("li", { class: "promise-item" }, [
      el("span", { class: "promise-icon", "aria-hidden": "true" }, [icon]),
  ...
  return el("ul", { class: "promise-list" }, [
    item("🔒", "End-to-end encrypted", "AES-256 in this tab. Your passphrase never leaves your device, so nobody else can read your files — not us, not your storage provider."),
    item("🪣", "Your storage, your keys", "A bucket in your own Cloudflare account, set up for you in one click — or bring your own (R2, Hetzner, Backblaze, AWS). We never hold your data and there's nothing to subscribe to."),
    item("🚫", "No account, no middleman", "No NakliTechie sign-up, no server on the path, no telemetry. Just your browser talking to your bucket."),
  ]);
}
...
export function createWizard({ root, onComplete, liveRegion } = {}) {
  ...
  function persistableSnapshot() {
    ...
    return {
      ...
      route: state.route,
      data: {
        // secret deliberately omitted — see pendingCarrier for where it lives.
        carrier: { url: state.data.carrier.url, verified: state.data.carrier.verified },
  ...
  async function loadSession() {
    try {
      ...
      if (!saved) return;
      if (saved.data?.carrier) {
        state.data.carrier.url = saved.data.carrier.url || "";
        // verified only holds if the secret is still around to back it
        state.data.carrier.verified = !!saved.data.carrier.verified && !!pendingCarrier.load();
      }
  ...
  function renderWelcome() {
    const carrierBtn = el("button", { type: "button", class: "btn btn-primary btn-block btn-lg" }, ["Set up with one click"]);
    carrierBtn.addEventListener("click", () => { state.route = "carrier"; go("carrier"); });
    const newFolderBtn = el("button", { type: "button", class: "btn btn-secondary btn-block btn-lg" }, ["Set up with your own bucket"]);
    newFolderBtn.addEventListener("click", () => { state.route = "new-folder"; go("bucket"); });
    const unlockBtn = el("button", { type: "button", class: "btn btn-secondary btn-block btn-lg" }, ["Unlock an existing folder"]);
    unlockBtn.addEventListener("click", () => { state.route = "unlock"; go("unlock"); });

    const helpLink = el("button", { type: "button", class: "btn-link", "aria-haspopup": "dialog" }, [
    ...
    return makeFragment(
      el("section", { class: "landing", "aria-labelledby": "stage-title" }, [
        el("div", { class: "landing-hero" }, [
          el("h1", { id: "stage-title", class: "landing-wordmark" }, ["Crate"]),
        ...
        promiseList(),
        el("div", { class: "landing-meta landing-prereqs" }, [
          el("p", { class: "prereqs-title" }, ["Before you start — you'll need:"]),
          prerequisitesList(),
        ]),
        el("div", { class: "landing-actions" }, [carrierBtn, newFolderBtn, unlockBtn]),
        el("p", { class: "muted small landing-alt" }, ["About a minute, start to finish. Already have a bucket and an API token? Use ", el("b", {}, ["Set up with your own bucket"]), "."]),
        el("p", { class: "landing-help" }, [helpLink]),
        el("hr", { class: "stage-divider landing-footer" }),
  ...
  function openSetupGuide({ returnFocus = null } = {}) {
    ...
    const setupTitle = el("h2", { id: "setup-title", tabindex: "-1" }, ["What you'll need + what each step does"]);
  ...
  // is, the privacy promise, and how it works in three steps. This is the
  // modal that auto-opens once on a first visit (see maybeAutoOpenHelp);
  ...
  function openHelpModal({ returnFocus = null } = {}) {
    ...
    card.appendChild(step(1, "Connect storage you own", [
      el("p", {}, [
        "Point Crate at a bucket in your own Cloudflare account (or Hetzner, Backblaze, AWS). The free tier is plenty for most folders. The setup guide gives you the exact settings to paste — about 3 minutes, once.",
      ]),
    ]));
    ...
    card.appendChild(step(3, "Open it from anywhere", [
      el("p", {}, [
        "Drop files in. Open this same link on your laptop or phone, unlock with your passphrase, and your folder is there. Your storage provider only ever sees encrypted bytes.",
      ]),
    ]));

    card.appendChild(el("hr", { class: "stage-divider" }));
    card.appendChild(el("p", { class: "muted small" }, [
      "First time setting up a storage bucket? The full setup guide walks through Cloudflare step by step.",
    ]));
  ...
  function renderCarrier() {
    ...
    if (!c.secret) {
      ...
      if (pending) { c.secret = pending.secret; if (!c.url && pending.url) c.url = pending.url; }
      else { c.secret = randomCarrierSecret(); pendingCarrier.save({ secret: c.secret, url: c.url }); }
    }
    const pill = makePill();
    ...
    urlInput.addEventListener("input", () => {
      ...
      pendingCarrier.save({ secret: c.secret, url: c.url });
        ...
        el("ol", { class: "steps" }, [
          el("li", {}, [
            el("div", { class: "field" }, [
              ...
              el("p", { id: "carrier-secret-help", class: "field-help muted" }, ["Generated just now, for you. The deploy form asks for it as ", el("code", {}, ["CARRIER_SECRET"]), "."]),
            ]),
          ]),
          el("li", {}, [
            el("p", {}, ["2 · Deploy — pick your GitHub account, paste the secret into ", el("code", {}, ["CARRIER_SECRET"]), ", leave the rest as suggested, click ", el("b", {}, ["Deploy"]), ". The build takes about a minute."]),
            makeDeepLinkButton(CARRIER_DEPLOY_URL, "Deploy to Cloudflare ↗"),
          ]),
          el("li", {}, [
            el("p", {}, ["3 · When it's done, click ", el("b", {}, ["Visit"]), " and then ", el("b", {}, ["Continue to Crate"]), " — the Worker's address fills in below. Or paste it."]),
            el("div", { class: "field" }, [
  ...
  function renderDone() {
    ...
    if (status === "running") {
      ...
      const ackText = el("span", {}, [
        ...
        el("code", {}, [".crate-creds"]),
        " file or the 5 bucket strings written down (in ",
        el("a", { href: "https://tijori.naklitechie.com/", target: "_blank", rel: "noopener" }, ["Tijori"]),
      ...
      const dlCredsBtn = el("button", { type: "button", class: "btn btn-secondary btn-block" }, [
        "↓ Download credentials file" + (state.handoff ? "" : " (recommended)"),
      ]);
      ...
      el("section", { class: "stage stage-done" }, [
        ...
        status === "done"
          ? el("div", { class: "hint-card" }, [
              ...
              el("p", { class: "muted small", style: "margin: 0;" }, [
                "Download a small encrypted credentials file. With this file + your passphrase, you can unlock from any device in two clicks instead of typing five strings. The file is useless without your passphrase. Keep it like a backup credential — store it in ",
                el("a", { href: "https://tijori.naklitechie.com/", target: "_blank", rel: "noopener" }, ["Tijori"]),
  ...
  // maybeAutoOpenHelp shows the "What is Crate?" explainer exactly once,
  // on a visitor's first landing. Guards: only on the welcome stage (not
  // when a hash or an in-tab session routed us to unlock), and only when
  // the localStorage flag is unset. The flag is written before opening so
  // a mid-open error can't make the modal reappear on the next load.
  function maybeAutoOpenHelp() {
    if (state.stage !== "welcome") return;
    let seen = null;
    try { seen = localStorage.getItem(HELP_SEEN_KEY); } catch { return; }
    if (seen) return;
    try { localStorage.setItem(HELP_SEEN_KEY, String(Date.now())); } catch {}
    const cta = root.querySelector(".landing-actions .btn-primary");
    openHelpModal({ returnFocus: cta || null });
  }
    ...
    return loadSession().then(() => {
      ...
      if (carrierLink) {
        ...
        if (validCarrierUrl(url)) {
          ...
          const pending = pendingCarrier.load();
          if (pending) { state.data.carrier.secret = pending.secret; pendingCarrier.save({ secret: pending.secret, url }); }
          state.stage = "carrier";
