...
// the 7 new-folder stages (welcome → bucket → credentials → cors →
// passphrase → recovery → done), the 2 pair-device stages (pair-unlock
// → pair-done), and the unlock-existing-folder stage.
//
// The entropy meter on the Passphrase stage is real (zxcvbn-ts via
// lib/entropy.js). The recovery phrase is real BIP-39 (lib/wordlist.js +
// crypto.getRandomValues). Passphrase and recovery phrase are
// memory-only — they NEVER touch IndexedDB or localStorage.
...
//
// Encodes 256 bits of entropy + 8 bits SHA-256 checksum = 264 bits = 24×11.
...
// Synthetic check used by the provisional pair-device flow (renderPairUnlock).
// The new-folder + unlock routes use real bucket calls; this stub gives
// the pair-via-token UI plausible feedback until that flow's full wiring
// lands. See docs/esm-api.md for v1 contract.
...
export function createWizard({ root, onComplete, liveRegion } = {}) {
  ...
  function renderRecovery() {
    ...
    return makeFragment(
      el("section", { class: "stage stage-recovery" }, [
        ...
        el("p", { class: "muted" }, ["These 24 words are your backup credential. They encode the same entropy as your passphrase; a future Crate version will let you set a new passphrase using them. Write them on paper. Do not screenshot them. Do not email them to yourself. Crate cannot recover them for you."]),
        grid,
  ...
  function renderPairUnlock() {
    ...
    return makeFragment(
      el("section", { class: "stage stage-pair-unlock" }, [
        ...
        el("p", { class: "muted small" }, ["Copy the token from the source device and paste it here. (A camera-based QR scanner is on the roadmap; for now the token is paste-only.)"]),
        el("div", { class: "field" }, [
  ...
  function renderPairDone() {
    if (root) root.setAttribute("data-test-complete", "true");
    const backBtn = el("button", { type: "button", class: "btn btn-primary btn-block" }, ["Open the folder via Unlock"]);
    backBtn.addEventListener("click", () => { state.route = "unlock"; go("unlock"); });
    return makeFragment(
      el("section", { class: "stage stage-pair-done" }, [
        el("h2", { id: "stage-title" }, ["Token accepted"]),
        el("p", { class: "lead" }, ["For v1, the most reliable way to open the folder on this device is the Unlock route — enter the bucket creds + passphrase and you're in."]),
        el("div", { class: "stage-actions" }, [backBtn]),
      ]),
    );
  }
