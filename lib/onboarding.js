...
export function createWizard({ root, onComplete, liveRegion } = {}) {
  ...
  const state = {
    ...
    data: {
      ...
      passphrase: { value: "", confirmed: false, mode: "suggested", suggested: "", saved: false }, // memory-only
      unlock: {
  ...
  function reset() {
    ...
    state.data.passphrase = { value: "", confirmed: false, mode: "suggested", suggested: "", saved: false };
    state.data.unlock = {
      mode: "file",
      fileBytes: null, fileHint: null, fileFromSession: false,
      bucketName: "", accountId: "",
  ...
  function renderWelcome() {
    ...
    return makeFragment(
      el("section", { class: "landing", "aria-labelledby": "stage-title" }, [
        ...
        el("p", { class: "muted small landing-alt" }, ["The first time, Cloudflare asks to connect your GitHub account so it can copy Crate's helper into it — allow a few minutes for that; the deploy itself takes about one. Already have a Cloudflare R2 bucket and an API token? Use ", el("b", {}, ["Set up with your own bucket"]), "."]),
  ...
  function openHelpModal({ returnFocus = null } = {}) {
    ...
    card.appendChild(step(3, "Open it from anywhere", [
      el("p", {}, [
        "At the end of setup, download your .crate-creds file and keep your passphrase. On any other device: open this same link, choose Unlock an existing folder, pick that file, enter your passphrase — and your folder is there. Your storage provider only ever sees encrypted bytes.",
      ]),
    ]));

    card.appendChild(el("hr", { class: "stage-divider" }));
    card.appendChild(el("p", { class: "muted small" }, [
      "Setting up with your own bucket instead? The full setup guide walks through Cloudflare's dashboard step by step.",
    ]));
  ...
  // The passphrase stage opens with seven generated words. Most people
  ...
  // Same 70-bit floor either way — the generated words clear it by design.
  ...
  function renderSuggestedPassphrase() {
    ...
    const chips = el("div", { class: "words", role: "group", "aria-label": "Your passphrase, seven words" },
    ...
    againBtn.addEventListener("click", () => {
      ...
      render();
      announce(liveRegion, "New seven-word passphrase shown.");
    });
    ...
    return makeFragment(
      el("section", { class: "stage stage-passphrase" }, [
        ...
        el("p", { class: "muted" }, [
          "These seven words unlock your folder. Write them down now — on paper, or in your password manager. Nobody can recover them for you, and Crate never stores them.",
        ]),
        chips,
        el("div", { class: "row" }, [copyBtn, againBtn]),
        el("label", { class: "ack", for: "pp-saved" }, [box, el("span", {}, ["I've saved these seven words somewhere I'll find them again."])]),
        el("p", { class: "muted small" }, [ownLink]),
        el("p", { class: "muted small" }, ["Why seven words: it's about 77 bits of strength — comfortably past the 70 Crate requires — and words are far easier to write down correctly than symbols."]),
      ]),
    );
  }
  ...
  function renderOwnPassphrase() {
    ...
    const backLink = el("button", { type: "button", class: "btn-link" }, ["← Use the suggested seven words instead"]);
    ...
    return makeFragment(
      el("section", { class: "stage stage-passphrase" }, [
        ...
        el("p", { class: "muted" }, [
          "This unlocks your folder. Crate cannot reset or recover it, so save it somewhere you'll find it again. The bar below turns green when it's strong enough (",
          el("strong", {}, [`${MIN_BITS}`]),
          " bits); enter the same passphrase in both fields.",
        ]),
        el("div", { class: "hint-card" }, [
          el("div", { class: "hint-card-title" }, ["What clears the bar:"]),
          el("ul", {}, [
            el("li", {}, [
              el("strong", {}, ["Memorable: 6 unrelated common words."]),
              " e.g. ",
              el("code", {}, ["correct-horse-battery-staple-lamp-river"]),
              " (~75 bits). Four words is ~50 and won't pass; keep adding words until the bar is green.",
            ]),
            el("li", {}, [
