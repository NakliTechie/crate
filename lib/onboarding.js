...
// Run the synthetic check on a string. Empty or literal "fail" → fail;
// anything else → ok. Promise resolves after a randomized delay so the
// pill spends visible time in "Checking…".
...
export function createWizard({ root, onComplete, liveRegion } = {}) {
  ...
  function renderBucket() {
    ...
    verifyBtn.addEventListener("click", async () => {
      ...
      pill.check();
      const ok = state.data.bucket.name && state.data.bucket.accountId;
      const failTrip = [state.data.bucket.name, state.data.bucket.accountId].some(v => (v ?? "").trim().toLowerCase() === "fail");
      const r = await fakeVerify(failTrip ? "fail" : (ok ? "ok" : ""));
      if (r.ok) {
        pill.ok("✓ Bucket reachable");
        state.data.bucket.verified = true;
      } else {
        pill.fail("✗ Bucket unreachable");
        state.data.bucket.verified = false;
      }
    ...
    return makeFragment(
      el("section", { class: "stage stage-bucket" }, [
        ...
        el("p", { class: "muted small" }, ["Tip: type ", el("code", {}, ["fail"]), " in either input to see the error path."]),
      ]),
    );
  }
  ...
  function renderCredentials() {
    ...
    testBtn.addEventListener("click", async () => {
      ...
      pill.check();
      const ok = state.data.credentials.accessKey && state.data.credentials.secretKey;
      const failTrip = [state.data.credentials.accessKey, state.data.credentials.secretKey].some(v => (v ?? "").trim().toLowerCase() === "fail");
      const r = await fakeVerify(failTrip ? "fail" : (ok ? "ok" : ""));
      if (r.ok) {
        pill.ok("✓ Credentials authenticated");
        state.data.credentials.verified = true;
      } else {
        pill.fail("✗ Authentication failed");
        state.data.credentials.verified = false;
      }
  ...
  function renderCors() {
    ...
    preflightBtn.addEventListener("click", async () => {
      ...
      const r = await fakeVerify("cors");
      if (r.ok) {
        pill.ok("✓ Preflight succeeded");
        state.data.cors.preflighted = true;
      } else {
        pill.fail("✗ Preflight failed (CORS not configured)");
        state.data.cors.preflighted = false;
      }
  ...
  function renderRecovery() {
    ...
    return makeFragment(
      el("section", { class: "stage stage-recovery" }, [
        ...
        el("p", { class: "muted" }, ["These 24 words can restore access if you forget your passphrase. Write them on paper. Do not screenshot them. Do not email them to yourself. Crate cannot recover them."]),
        grid,
