...
export function parseShare(fragment) {
  ...
  return {
    ...
    uuid: p.u, key: unb64url(p.k), contentIv: p.iv, chunkSize: p.cs ?? undefined, exp: Number(p.exp) || 0, url: p.url,
  };
}
...
export async function fetchShared(share, { signal } = {}) {
  ...
  try {
    return await cryptoLib.openObject(share.key, body, { uuid: share.uuid, content_iv: share.contentIv, chunk_size: share.chunkSize, size: share.size });
  } catch (e) {
