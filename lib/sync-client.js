...
// Sync — M6. Keeps multiple tabs of the same origin AND multiple devices
...
//
// Concurrent-write safety: out of scope for v1.0. Two tabs PUTting the
// manifest at the same time → last writer wins; the loser sees their
// next poll snap forward to the winner's tree. ETag-conditional PUT
// with replay-on-412 is M6.x.
...
export class SyncClient {
  ...
  async _pollManifest() {
    ...
    // folder UI, refresh runner) sees the new state. The Manifest class
    // exposes `events` + `_lastSig` as plain fields we can overwrite.
    // M6 v1.0 assumes the Crate's local manifest is flushed after every
    // mutation, so there are no pending local-only events to preserve.
...
//
// (Moves are detected as a {delete old, create new} pair; M6 v1.0 doesn't
// try to coalesce.)
