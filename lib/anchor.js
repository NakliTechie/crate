...
export async function loadAnchor(bucketBase) {
  ...
  try {
    ...
    if (v && typeof v.count === "number" && typeof v.lastSig === "string") {
      return { count: v.count, lastSig: v.lastSig, generation: v.generation || 0, source: "idb" };
    }
  } catch (_e) {
  ...
  try {
    ...
    if (raw) {
      ...
      if (v && typeof v.count === "number" && typeof v.lastSig === "string") {
        return { count: v.count, lastSig: v.lastSig, generation: v.generation || 0, source: "session" };
      }
    }
  } catch (_e) {
...
export async function saveAnchor(bucketBase, anchor) {
  ...
  const key = await computeAnchorKey(bucketBase);
  const payload = { count: anchor.count, lastSig: anchor.lastSig, generation: anchor.generation || 0, ts: Date.now() };
  // IndexedDB primary.
  try {
...
export function validate(loadedEvents, prior) {
  const generation = generationOf(loadedEvents);
  const loaded = {
  ...
  // `rekey` event (it needs the new key, which crate.json now yields only
  // to the passphrase / recovery phrase), and replaying the pre-re-key
  // bucket state brings the old generation back — refused.
  if (generation > (prior.generation || 0)) {
    return { ok: true, anchor: loaded, rekeyed: true };
  }
  ...
  return { ok: true, anchor: loaded };
}

function generationOf(events) {
  let g = 0;
  for (const e of events) if (e?.op === "rekey" && Number.isInteger(e.generation) && e.generation > g) g = e.generation;
  return g;
}
